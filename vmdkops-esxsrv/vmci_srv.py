...
make sure mkfs.ext4 is installed on ESX and make sure backend name is properluy calculated - 
...
==
make sure the NEW disk is really formatted
== 
...
# attaches *existin* disk to a vm                              
def disk_attach(vmdkPath, vm):                             
  # get all disks on a VM, set unit_number to the next available
  # *** this code will NOT work on multuple scsci controllers   
  #*** plus we need to use type(dev) == vim.vm.device.ParaVirtualSCSIController
  # to avoid scsi bus rescan on attach                                         
  #                                                                            
  # here is what we really need to do :                                        
  # - check if 4 pvsci is there. If not - create                               
  # - find a slot on pvsci (I guess need to check for 7 and if its' reserve, exclude)
  # - if no slots, bail out                                                          
  # for now, assume pvsci exists and look for a slot on it                           
                                                                                     
  pvsci = [dev for dev in vm.config.hardware.device                                  
                                  if type(dev) == vim.vm.device.ParaVirtualSCSIController]
                                                                                          
  if len(pvsci) < 1:                                                                      
          raise StandardError("Pvsci adapter is missing - please add to docker VM ")      
                                                                                          
  # get all slots taken on controller (id's by key) and find the first empty one.         
  # we need to search in all 4 pvsci when we ad them automatically, for now search in the first one only
  key = pvsci[0].key                                                                                    
  taken = set([dev.unitNumber for dev in vm.config.hardware.device if                                   
          type(dev) == vim.vm.device.VirtualDisk and dev.controllerKey == key])                         
  # supposedly unit_number 7 reserved for scsi controller                                               
  allSlots = set(range (0,6) + range (8,16))                                                            
  availSlots = allSlots - taken                                                                         
  if len(availSlots) == 0:                                                                              
          raise StandardError("We don't support this many disks yet")                                   
  slot = availSlots.pop()                                                                               
  print "bus=" , pvsci[0].busNumber, " key=", key, " slot=", slot                                       
  ...
  disk_spec.device.deviceInfo.label = "dockerDataVolume"  
  # TODO: Use docker data volume name for label - this way we use it on detach 
  disk_spec.device.deviceInfo.summary = "dockerDataVolume"                        
                                                                                  
  disk_spec.device.backing.fileName = "[] " + vmdkPath                            
  disk_spec.device.backing.diskMode = 'persistent'                                
                                                                                  
  disk_spec.device.unitNumber = slot                                              
  disk_spec.device.controllerKey = key                                            
